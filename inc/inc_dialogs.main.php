...
<div id="dialog_certificado" title="<? echo "Certificado" ?>">
	<form method="GET" action="../certificado/tablas.php" target="_blank">
		<div class="row">
			<div class="row2">
				<div class="width100">
					<input class="inputbox" type='text' name='patente' maxlength="100"/>
				</div>
			</div>
		</div>
			
		<center>
			<input class="button icon-search icon" type="submit" value="<? echo $la['SEARCH']; ?>" />&nbsp;&nbsp;&nbsp;
	</form>
			<input class="button icon-close icon" type="button" onclick="$('#dialog_certificado').dialog('close');" value="<? echo $la['CANCEL']; ?>" />
		</center>	
</div>
