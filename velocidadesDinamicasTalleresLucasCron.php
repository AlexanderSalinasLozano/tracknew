<?php
	...
	echo 'iniciando reproceso en fecha desde : '.$finicio.' hasta '.$ffinal;
	echo '</br>';
	$mysqli = new mysqli('10.118.144.7', 'stech', 'Stech#306..,', 'gpsimple');
	$q  = "select gs_objects.imei from gs_objects, gs_user_objects where gs_objects.imei = gs_user_objects.imei and gs_user_objects.user_id = 1019";
	...
	while($row = mysqli_fetch_row($r)){
		...
		while($row2 = mysqli_fetch_row($r2)){
			...
			echo $imei;
				...
				$url = 'http://10.128.15.198/api/interpreter?data=[out:json];way(around:05.0,'.$lat.','.$lng.');out;';
