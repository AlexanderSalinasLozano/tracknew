<?
    ...
    /* include ($gsValues['PATH_ROOT'].'config.custom.php'); */
	...
	function insert_db_loc($loc)
	{
		...
		//isset()
		$loc['overpass'] = isset($loc['overpass']) ? $loc['overpass'] : '0';
		//empty()
		$loc['overpass'] = !empty($loc['overpass']) ? $loc['overpass'] : '0';

		//isset()
		$loc['event'] = isset($loc['event']) ? $loc['event'] : ' ';
		//empty()
		$loc['event'] = !empty($loc['event']) ? $loc['event'] : ' ';		
		...
		if($loc['imei'] == '864403045739738'||$loc['imei'] == '864403041379604'||$loc['imei'] == '864403041362626'||$loc['imei'] == '867060037426562'||$loc['imei'] == '867060037738883' ||$loc['imei'] == '864403044863554'||$loc['imei'] == '12345678901'){
	...
	function actualizar_gs_objects($imei)
	{

		global $ms;

		$k= "SELECT * FROM gs_objects WHERE `imei`='".$imei."'";
		$r = mysqli_query($ms, $k) or die(mysqli_error($ms));
	...
	$loc_actualiza = actualizar_gs_objects($loc['imei']);
		...
		insert_db_objects($loc, $loc_actualiza);
		...
		function SelectMaxSpeed($imei)
		{
			global $ms;
			
			$speedBase = 0;

			$q = "SELECT `max_speed` FROM gs_objects WHERE `imei`='".$imei."'";
		...
		function insertMaxSpeed($loc){
			
			global $ms;
			// var_dump("saludos desde la funcion");
			$imei = $loc['imei'];	
			$speedReal = $loc['speed'];
			$speedBd = SelectMaxSpeed($imei);
			//    var_dump("speed real: ".$speedReal);
			//    var_dump("speed bd: ".$speedBd);
				if($speedReal > $speedBd){

					$q = "UPDATE gs_objects SET `max_speed`='".$speedReal."'
					WHERE imei='".$loc['imei']."'";
		...
		if (loc_filter($loc, $loc_prev) == false)
		{
			...
			if (($loc['lat'] != 0) && ($loc['lng'] != 0))
			{
				...
				if (strtotime($loc['dt_tracker']) > strtotime($loc_prev['dt_tracker']))
				{
					var_dump('FECHA ES MAYOR');
					check_events($loc, true, true, true,false);
					insertMaxSpeed($loc);
				}
				else
				{   
					var_dump('FECHA ES MENOR');
	...
	function insert_db_objects($loc, $loc_actualiza)
	{
		global $ms;
        
		$sate = 1;
		if (isset($loc['satelites'])) {
			$sate=$loc['satelites'];
		}
		...
		if (strtotime($loc['dt_tracker']) > strtotime($loc_actualiza))
		{           
			if ($loc['loc_valid'] == 1)
			{
			...
			//calculate angle
				// if ($loc['angle'] == 0)
				// {
				// 	$loc['angle'] = getAngle($loc_actualiza['lat'], $loc_actualiza['lng'], $loc['lat'], $loc['lng']);	
				// }
	...
	function insert_db_object_data($loc)
	{
		...
		if (($loc['lat'] < 0))
		{
		   if (($loc['lng'] < 0))
		   {
			...
			$user2="root";
			$password2="Stech3162..,";
			$dbname2="data_produccion";
			
			$conn2 = mysqli_connect($host2, $user2, $password2, $dbname2, $port2, $socket2)
			...
			$sql = "CALL `insertaDataGps`(
				...
				//OJO aqui ingresa a backup activar en prod
				if(false){
					$consulta = mysqli_query($conn2, $sql) or die(mysqli_error($conn2));
				}
	...
	function loc_filter($loc, $loc_prev)
	{
		...
		*/
		if (($loc['dt_tracker'] == $loc_prev['dt_tracker']) && ($loc_prev['lat'] == $loc['lat']) && ($loc_prev['lng'] == $loc['lng']))
		{
			return true;			
		...
		{
			if (($loc_prev['params'] == $loc['params']))
			{
				$dt_difference = abs(strtotime($loc['dt_tracker']) - strtotime($loc_prev['dt_tracker']));
