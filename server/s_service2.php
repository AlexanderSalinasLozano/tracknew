<?php
   ...
   var_dump('EJECUCION S_SERVICE2');
    ...
    //    echo "<script>alert('MENSAJE DE ALERTA S_SERVICE')</script>";
...
//				continue;
    ...
    function serviceAppNotificacionGeocerca() {
        //Se llama a la variable conexion de la Base de Datos
        global $ms;
        //Esta es la query que llamara a las geocercas activas
        $queryZones = "SELECT * FROM `gs_user_zones` WHERE (zone_name='App Geocerca' and zone_visible='true')";
        //Se ejecuta la query
        $resultadoZones = mysqli_query($ms, $queryZones);

        //Se obtienen los datos entregados por la query y se guardan en un Array
        while($fila = $resultadoZones->fetch_array()){
        $filasGeocercasActivas[] = $fila;
        }

        //Se recorre el array de geocercas creanfo $filaGeo en cada Ciclo
        foreach($filasGeocercasActivas as $filaGeo){
            //Se obtienen los datos de filaGeo
            $imei = $filaGeo['imei'];
            $user_id = $filaGeo['user_id'];
            //Se separan la latitud y longitud separadas por una ",(coma)" en la base de datos
            $geocerca = explode(",", $filaGeo['vertices']);
            //Se nombran las las variables del array "$geocerca" recien creado para mejor comprension
            $latGeocerca = $geocerca[0];
            $lngGeocerca = $geocerca[1];

            //Se llama a la latitud y longitud del vehiculo con el imei obtenido en la tabla gs_objects
            $queryObjects = "SELECT lat,lng FROM `gs_objects` WHERE imei=$imei";
            //Se ejecuta la query
            $resultadoObjects = mysqli_query($ms, $queryObjects);

            //Se obtienen los datos entregados por la query
            //FIXME: While Innecesario porque se obtienen los datos una sola vez
            while($fila = $resultadoObjects->fetch_array()){
                $latMarker = $filaObjects[lat];
                $lngMarker = $filaObjects[lng];
                $vehiculo = $latMarker.",".$lngMarker;
                //Se restan los valores absolutos(se pasan a positivos) de la latitud/longitud del geocerca y del vehiculo
                $latDiferencia = abs[$latGeocerca] - abs[$latMarker];
                $lngDiferencia = abs[$lngGeocerca] - abs[$lngMarker];
                //Si la diferencia es mayor a 0.00045045 quiere decir que el vehiculo se movio mas de 50 metros
                if($latDiferencia >=0.00045045 || $lngDiferencia >=0.00045045 ){
                    //Se buscan los tokens registrados en la Base de Datos del id del usuario
                    $queryTokens = "SELECT token FROM `gs_user_tokens` WHERE id_usuario=$user_id";
                    $resultadoTokens = mysqli_query($ms, $queryTokens);

                    //Se obtienen los tokens y se guardan en $tokens
                    while($filaToken = $resultadoTokens->fetch_array()){
                        $tokens[] = $filaToken;
                    }
                    //TODO: Hacer metodo que ejecuta el envio de notificacion a Firebase

                    //2) Registrar en la base de datos el envio de la notificacion
                    $queryTokenHistorial = "INSERT INTO `gs_user_tokens_historial` VALUES (0, $imei, $geocerca, $vehiculo, $user_id)";
                    $resultadoTokens = mysqli_query($ms, $queryTokens);
                        }
            }
        }
